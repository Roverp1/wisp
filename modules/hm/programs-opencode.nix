...
in
{
  ...
  options.programs.opencode = {
    ...
    tui = mkOption {
      ...
      description = ''
        ...
        Since OpenCode v1.3.4, TUI settings must be in a separate tui.json file.
        Settings like `theme` and `keybinds` in {option}`programs.opencode.settings`
  ...
  config = mkIf cfg.enable {
    ...
    warnings =
      ...
      in
      lib.optionals (deprecatedTuiKeys != [ ]) [
        ''
          programs.opencode.settings contains deprecated TUI-specific keys: ${lib.concatStringsSep ", " deprecatedTuiKeys}

          These settings should be moved to programs.opencode.tui instead.

          OpenCode v1.3.4+ requires TUI settings in a separate tui.json file.
